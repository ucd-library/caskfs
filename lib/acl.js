...
class Acl {
  ...
  async all(pgClient) {
    let res = await pgClient.query(`SELECT * FROM ${config.pgSchema}.directory_acl_view`);
    return res.rows;
  }
  ...
  async set(directory, roles, pgClient) {
    ...
    let resp = await pgClient.query(
      `INSERT INTO ${config.pgSchema}.directory_acl (directory_id, read, write)
       VALUES ($1, $2, $3)
    ...
    await pgClient.query(`UPDATE ${config.pgSchema}.directory SET directory_acl_id = $1 WHERE directory_id = $2`, [aclId, directoryId]);
  ...
  async _setChildren(directoryId, aclId, pgClient) {
    ...
    let res = await pgClient.query(`SELECT directory_id FROM ${config.pgSchema}.directory_acl_view WHERE parent_id = $1 AND is_explicit = FALSE`, [directoryId]);
