...
class Database {
  ...
  async insertFile(filePath, hashId, metadata={}, partitionKeys=[]) {
    let fileParts = path.parse(filePath);
    let resp = await this.client.query(`
      SELECT * FROM ${this.schema}.insert_file(
        ...
        p_hash_id := $3::UUID,
        p_metadata := $4::JSONB,
        p_partition_keys := $5::TEXT[]
      ) AS file_id
    `, [fileParts.dir, fileParts.base, hashId, metadata, partitionKeys]);
  ...
  async getChildDirectories(directory, opts={}) {
    let res = await this.client.query(`
      ...
      SELECT * FROM ${config.pgSchema}.directory 
      WHERE parent_id = (select directory_id from dir) 
      LIMIT $2 OFFSET $3
      ORDER BY fullname ASC;
    `, [directory, opts.limit || 100, opts.offset || 0]);
  ...
  async findContainments(opts={}) {
    ...
    if( nodeWhere.length > 0 ) {
      nodeQuery = `UNION
      SELECT DISTINCT file_id FROM ${config.pgSchema}.rdf_node_view
      WHERE ${nodeWhere.join(' AND ')}`;
    }

    let resp = await this.dbClient.query(`
      with files as (
        SELECT DISTINCT file_id FROM ${config.pgSchema}.rdf_link_view
      ...
      SELECT 
        fv.*
      FROM files f
      JOIN ${config.pgSchema}.file_view fv ON fv.file_id = f.file_id
  ...
  async findRdfNodes(opts={}) {
    ...
    if( !opts.object ) {
      nodes = await this.dbClient.query(`
        SELECT * FROM ${config.pgSchema}.rdf_node_view WHERE ${where.join(' AND ')} ${limit}
      `, args);
      nodes = nodes.rows;
    }

    let links = await this.dbClient.query(`
      SELECT * FROM ${config.pgSchema}.rdf_link_view WHERE ${where.join(' AND ')} ${limit}
    `, args);
    links = links.rows;
