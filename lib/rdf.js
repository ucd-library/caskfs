...
class Rdf {
  ...
  async insert(fileId, opts={}) {
    ...
    await dbClient.query(`select caskfs.insert_rdf_link_bulk($1::UUID, $2::JSONB)`, [fileId, JSON.stringify(otherQuads)]);
    await dbClient.query(`select caskfs.insert_rdf_node_bulk($1::UUID, $2::JSONB)`, [fileId, JSON.stringify(nodes)]);
